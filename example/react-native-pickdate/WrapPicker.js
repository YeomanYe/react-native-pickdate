...
export class WrapIosDatePicker extends Component {
    ...
    static _genDatePicker(paramObj){
        ...
        switch (type){
            case DATE_TYPE.TIME:paramObj.titleText = 'Pick a date';break;
            case DATE_TYPE.DATETIME:
            case DATE_TYPE.DATE:paramObj.titleText = 'Pick a time';break;
        }
        return (
...
export class WrapAndroidDatePicker{
   static async showPicker({type,onConfirm,onCancel,date,is24Hour}){
       type = type || DATE_TYPE.DATE;
       let retTime,retDate;
       if(type === DATE_TYPE.DATE){
           retDate = await WrapAndroidDatePicker._showDatePicker(date);
           if(onConfirm && retDate) onConfirm(new Date(retDate));
           else if(onCancel) onCancel();
       }
       else if (type === DATE_TYPE.DATETIME){
           retDate = await WrapAndroidDatePicker._showDatePicker(date);
           retTime = await WrapAndroidDatePicker._showTimePicker(date,is24Hour);
           if(retDate && retTime && onConfirm) onConfirm(new Date(retDate+' '+retTime));
           else if(onCancel) onCancel();
       }else{
           retTime = await WrapAndroidDatePicker._showTimePicker(date,is24Hour);
           retDate = '1970-01-01';
           if(onConfirm && retTime) onConfirm(new Date(retDate + ' ' + retTime));
       }
    }
